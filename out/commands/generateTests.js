...
async function generateTestsCommand() {
    ...
    const jiraKey = await vscode.window.showInputBox({
        ...
        ignoreFocusOut: true,
        validateInput: (v) => /^[A-Z]+-\d+$/.test(v.trim())
            ? null
    ...
    const summary = await vscode.window.showInputBox({
        ...
        ignoreFocusOut: true,
        validateInput: (v) => v.trim().length >= 10
            ? null
    ...
    const description = await vscode.window.showInputBox({
        ...
        prompt: 'Enter the full JIRA story description (acceptance criteria, compliance notes, etc.)',
        placeHolder: 'As a compliance officer, I need all wire transfers over $10,000 to be screened ' +
        ...
        ignoreFocusOut: true,
        validateInput: (v) => v.trim().length >= 20
            ? null
    ...
    await vscode.window.withProgress({
        ...
        try {
            ...
            const trigger = await client.triggerWorkflow(jiraKey.trim(), summary.trim(), description.trim());
            ...
            const result = await client.pollWorkflow(workflowId, pollInterval, pollTimeout, (update) => {
                if (update.status !== lastStatus) {
                    ...
                    const msg = update.status === 'running'
            ...
            if (result.status === 'waiting_approval') {
                ...
                const checkpoint = approvals.length > 0 ? approvals[0].checkpoint : 'scenarios';
                const decision = await vscode.window.showInformationMessage(`Workflow ${workflowId} is waiting for approval at checkpoint: "${checkpoint}". Approve?`, 'Approve', 'Reject');
                if (decision === 'Approve' || decision === 'Reject') {
                    await client.submitApproval(workflowId, checkpoint, decision === 'Approve' ? 'approved' : 'rejected');
                    // Resume polling after approval
                    const finalResult = await client.pollWorkflow(workflowId, pollInterval, pollTimeout, () => { });
            ...
            vscode.window.showErrorMessage(`Fortress: test generation failed — ${(0, client_1.extractErrorMessage)(err)}`);
        }
    });
}
async function showResult(result, jiraKey) {
    if (result.status === 'failed') {
        vscode.window.showErrorMessage(`Fortress: workflow failed for ${jiraKey}. Check Pipeline 2 logs.`);
        return;
    }
    if (result.status !== 'completed') {
        vscode.window.showWarningMessage(`Fortress: workflow ended with status "${result.status}" for ${jiraKey}.`);
        return;
    }
    ...
    const choice = await vscode.window.showInformationMessage(`Fortress: tests generated for ${jiraKey} (quality score: ${qualityScore})`, 'Open Test Code', 'Show Artifacts');
    if (choice === 'Open Test Code') {
        if (testCode) {
            ...
            vscode.window.showWarningMessage('No test_code found in workflow artifacts. The generation agent may still be running.');
        }
    }
